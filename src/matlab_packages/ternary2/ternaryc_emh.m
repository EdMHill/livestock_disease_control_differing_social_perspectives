function [hd]=ternaryc_emh(c1,c2,c3,d,symbol,cbar_limits)
%FUNCTION H=TERNARYC_EMH(C1,C2,C3,D,SYMBOL) plots the four data vectors C1,C2,C3, 
% and D color-coded in a ternary plot. The optional parameter SYMBOL
% determines the symbol used in the scatter plot. The optional vector
% CBAR_LIMITS specifies the normalisation of the value d to the desired
% colourbar range
...
if nargin == 3
    ...
    % Set the range the colourmap should plot between
    if nargin<6
        miv=min(d);
        mav=max(d);
    else 
...
for i=1:length(c1)    
    ...
    if marker == 1
        ...
        hd(i)=plot(x,y,symbol,...
                   'color',map(in,:),...
                   'markerfacecolor',map(in,:));
    end
end
hold off
axis image
...
%         s(i,1:length(B))=B;
%     end
%     %set(hcb,'yticklabel',s,'fontsize',15,'ycolor','k','xcolor','k');
%     set(hcb,'fontsize',15,'ycolor','k','xcolor','k');
% end
%grid on
